...
def extract_audatex(pdf_bytes: bytes) -> dict:
    ...
    for line in lines:
        ...
        if re.match(r'TOTAL M\.O\. PINTURA', line.strip()):
            m2 = re.search(r'\$([\d,]+\.?\d*)', line)
            if m2: meta["total_mo_pintura"] = parse_price(m2.group(1))
        if re.match(r'TOTAL MATERIALES', line.strip()):
            m2 = re.search(r'\$([\d,]+\.?\d*)', line)
            if m2: meta["total_mat_pintura"] = parse_price(m2.group(1))
        if re.match(r'TOTAL PINTURA', line.strip()):
            m2 = re.search(r'\$([\d,]+\.?\d*)', line)
            if m2: meta["total_pintura"] = parse_price(m2.group(1))
    ...
    for line in lines:
        ...
        # PINTURA (check first — appears after MO on page boundary)
        ...
        # MO (2-line header with possible tarifa line in between)
        ...
        if state == MO_PREHDR:
            ...
            continue   # stay: absorb tarifa line
        if state == MO_WAIT:
...
# ══════════════════════════════════════════════════════════════════════════════
# EXCEL — una sola hoja, columnas: N° Orden | Sección | NR/Pos. | Descripción | Precio
# ══════════════════════════════════════════════════════════════════════════════
...
def build_excel(all_data: list, filenames: list) -> bytes:
    ...
    # ── column widths ──
    ws.column_dimensions["A"].width = 12   # N° Orden
    ws.column_dimensions["B"].width = 26   # Sección
    ws.column_dimensions["C"].width = 14   # NR / Pos.
    ws.column_dimensions["D"].width = 52   # Trabajo / Descripción
    ws.column_dimensions["E"].width = 16   # Precio

    # ── styles ──
    THIN   = Side(style="thin",   color="CCCCCC")
    THICK  = Side(style="medium", color="888888")
    BDR    = Border(left=THIN, right=THIN, top=THIN, bottom=THIN)
    BDR_GRP= Border(left=THIN, right=THIN, top=THICK, bottom=THIN)
    ...
    # fills
    HDR_FILL  = PatternFill("solid", fgColor="0F3460")   # dark blue  – header row
    MO_FILL   = PatternFill("solid", fgColor="D6E4F7")   # light blue – MO rows
    MO_ALT    = PatternFill("solid", fgColor="EBF3FB")   # lighter    – MO alt
    PIN_FILL  = PatternFill("solid", fgColor="FADADD")   # light red  – Pintura rows
    PIN_ALT   = PatternFill("solid", fgColor="FDEEF0")   # lighter    – Pintura alt
    SUB_FILL  = PatternFill("solid", fgColor="16213E")   # dark       – subtotals
    GRP_FILL  = PatternFill("solid", fgColor="1A1A2E")   # darkest    – order group header
    GRAND_FILL= PatternFill("solid", fgColor="0B0B1A")   # black      – grand total
    ...
    for fname, data in zip(filenames, all_data):
        ...
        mo      = data["mo"]
        pin     = data["pintura"]
        n_orden = meta.get("num_orden", "–")
        vehiculo= f"{meta.get('fabricante','')} {meta.get('modelo','')}".strip()
        ...
        c = ws.cell(row, 1,
            f"  N° ORDEN: {n_orden}   |   "
            f"Exp: {meta.get('expediente','–')}   |   "
            f"{vehiculo}   |   "
            f"VIN: {meta.get('vin','–')}   |   "
            f"Taller: {meta.get('taller','–')}")
        ...
        for i, item in enumerate(mo):
            ...
            bdr  = BDR_GRP if i == 0 else BDR

            ws.cell(row, 1, n_orden).alignment  = CTR
            ...
            p = ws.cell(row, 5, item["precio"])
            p.number_format = MONEY; p.alignment = RGT

            for col in range(1, 6):
                c = ws.cell(row, col)
                c.fill   = fill
                c.border = bdr if col == 1 else BDR
                if col == 1: c.font = Font(bold=True, size=9, color="0F3460")
                if col == 2: c.font = Font(size=9, color="0F3460")
                if col in (3, 4, 5): c.font = Font(size=9)
        ...
        if mo:
            ...
            ct = ws.cell(row, 1, f"  SUBTOTAL  Mano de Obra Hojal/Mecánica  ·  N° Orden {n_orden}")
            cv = ws.cell(row, 5, total_mo)
            ct.font = W_FONT_SM; ct.fill = SUB_FILL; ct.alignment = LFT; ct.border = BDR
            cv.font = W_FONT_SM; cv.fill = SUB_FILL; cv.alignment = RGT; cv.border = BDR
            cv.number_format = MONEY
            grand_mo_sum += total_mo
            row += 1
        ...
        for i, item in enumerate(pin):
            ...
            for col in range(1, 6):
                ...
                if col == 2: c.font = Font(size=9, color="8B0000")
                if col in (3, 4, 5): c.font = Font(size=9)
        ...
        # Pintura subtotal
        if pin:
            total_pin = meta.get("total_pintura", pin_sum)
            ws.row_dimensions[row].height = 16
            ws.merge_cells(f"A{row}:D{row}")
            ct = ws.cell(row, 1, f"  SUBTOTAL  Pintura de Carrocería  ·  N° Orden {n_orden}")
            cv = ws.cell(row, 5, total_pin)
            ct.font = W_FONT_SM; ct.fill = SUB_FILL; ct.alignment = LFT; ct.border = BDR
            cv.font = W_FONT_SM; cv.fill = SUB_FILL; cv.alignment = RGT; cv.border = BDR
            cv.number_format = MONEY
            grand_pin_sum += total_pin
            row += 1
    ...
    ws.merge_cells(f"A{row}:D{row}")
    ct = ws.cell(row, 1, "  GRAN TOTAL  (todos los expedientes)  ·  M.O. + Pintura")
    cv = ws.cell(row, 5, grand_mo_sum + grand_pin_sum)
    ct.font = Font(bold=True, size=11, color="FFFFFF")
    cv.font = Font(bold=True, size=11, color="FFFFFF")
    ct.fill = cv.fill = GRAND_FILL
    ct.alignment = LFT; cv.alignment = RGT
    ct.border = cv.border = Border(
...
with st.sidebar:
    ...
    st.markdown("**Número de orden:** Referencia Interna del PDF")
    st.markdown("**Excel:** Una sola hoja ordenada por N° Orden")
...
total_pin = sum(d["meta"].get("total_pintura", sum(x["precio"] for x in d["pintura"]))  for d in all_data)
...
for fname, data in zip(filenames, all_data):
    ...
    with st.expander(f"📄  N° Orden {n_orden}  —  {fname}", expanded=True):
        if show_meta:
            mc1, mc2, mc3 = st.columns(3)
            mc1.markdown(f"**N° Orden:** {n_orden}")
        ...
        with c2:
            ...
            if pin:
                ...
                df["Precio ($)"] = df["Precio ($)"].map("${:,.2f}".format)
                st.dataframe(df, use_container_width=True, hide_index=True)
                t = meta.get("total_pintura", sum(x["precio"] for x in pin))
                if "total_mo_pintura" in meta and "total_mat_pintura" in meta:
                    st.info(f"M.O. Pintura: **${meta['total_mo_pintura']:,.2f}** | "
                            f"Materiales: **${meta['total_mat_pintura']:,.2f}**")
                st.error(f"**Total Pintura: ${t:,.2f}**")
            else:
                st.warning("No se encontraron partidas.")
...
with cy:
    rows = []
    for fname, data in zip(filenames, all_data):
        n_orden = data["meta"].get("num_orden", "–")
        for e in data["mo"]:
            rows.append({"N° Orden": n_orden, "Sección": "Mano de Obra",
                         "NR/Pos.": e["nr"], "Descripción": e["descripcion"], "Precio": e["precio"]})
        for e in data["pintura"]:
            rows.append({"N° Orden": n_orden, "Sección": "Pintura",
                         "NR/Pos.": e["nr"], "Descripción": e["descripcion"], "Precio": e["precio"]})
